...
NDiplomacy = {
	...
	MARCH_BASE_DEVELOPMENT_CAP = 700, 				-- Countries with more total development than this cannot be made into a march
	...
	TRUCE_YEARS = 3, 								-- _DDEF_TRUCE_YEARS_; Years of Truce
	SCALED_TRUCE_YEARS = 15,						-- Additional years of truce based on % of warscore taken in war (100% warscore = full scaled truce years)
	...
	FABRICATE_CLAIM_COST_MODIFIER_PER_CLAIM = 0.5,
	...
	NUM_OF_GREAT_POWERS = 5,						-- The number of ordinary (i.e. non-leaving) Great Powers if RightsOfManDLC is enabled.
	...
	TRIBUTE_BASE_CASH = 0.25,						-- Tributary State: Part of yearly income given in tribute
	TRIBUTE_BASE_ADM = 0.1,						-- Tributary State: Part Adm tribute, mulitplied by total development
	TRIBUTE_BASE_DIP = 0.1,						-- Tributary State: Part Dip tribute, mulitplied by total development
	TRIBUTE_BASE_MIL = 0.1,						-- Tributary State: Base Mil tribute, mulitplied by total development
	TRIBUTE_MAX_MONARCH_POWER = 15.0,				-- Tributary State: Max Adm/Dip/Mil per Tributary
	TRIBUTE_BASE_MANPOWER = 0.5,					-- Tributary State: Part of yearly manpower given in tribute
...
NCountry = {
	...
	MONARCH_MAX_SKILL = 10,						-- Monarch skills will be chosen between MONARCH_MIN_SKILL and MONARCH_MAX_SKILL
	...
	STATE_MAINTENANCE_DISTANCE_FACTOR = 0.01,	-- distance
	...
	ESTATE_INFLUENCE_LEVEL_1 = 30,
	ESTATE_INFLUENCE_LEVEL_2 = 50,
	ESTATE_INFLUENCE_LEVEL_3 = 80,
	...
	MIN_DEVELOPMENT_FOR_GOV_RANK_2 = 200,		-- Need at least this much development to upgrade to government rank 2
	...
	PARLIAMENT_ISSUE_DURATION = 20,				-- in years
	...
	MAX_ACTIVE_POLICIES = 9,						-- how many active policies at once.
	...
	POWER_MAX = 1500,								-- how much power can be stored at maximum.
	...
	MAX_TOLERANCE_HERETIC = 5, 						-- maximum tolerance towards heretics
	MAX_TOLERANCE_HEATHEN = 5, 						-- maximum tolerance towards heathens
	...
	LAGGINGTECH_CORRUPTION = 0.25,					-- Corruption increase/year. Multiplied by (highest - lowest) tech level
	LAGGINGTECH_CORRUPTION_MAX = 2.0,				-- Max increase/year
	...
	MONTHS_TO_CORE_MINIMUM = 1,						-- Minimum amount of months it will take to core a province after all modifiers.
	...
	REVOLT_SIZE_DEVELOPMENT_MULTIPLIER = 0.4,		-- Multiplied with the province's development
	REVOLT_SIZE_BASE = 4,
	...
	BASE_TARIFF = 0.25,								-- Basic part of colonies income that goes to tariffs
	...
	DICTATORSHIP_TRADITION_FOR_MONARCHY = 25,		-- If republican tradition is lower than this on death of ruler, dictatorship turns into monarchy
	DICTATORSHIP_TRADITION_FOR_REPUBLIC = 30,		-- If republican tradition is this or higher on death of ruler, dictatorship turns into republic
	...
	INNOVATIVENESS_FIRST_REACHED_TECH_LEVEL = 5.0,  -- Innovativeness bonus to first country (or countries) to reach a certain tech level.
	INNOVATIVENESS_FIRST_PICKED_IDEA = 2.5,			-- Innovativeness bonus to first country (or countries) to pick an idea.
	INNOVATIVENESS_BEHIND_IN_TECH_PENALTY = -0.05, 	-- Innovativeness penalty if behind neighbors in tech (and not ahead of time).
	INNOVATIVENESS_ENABLE_ANY_BEHIND_IN_TECH_PENALTY = 1,	-- Enabled that if any tech is behind we apply the penalty. Turning this off will require you only to be ahead in one.
	INNOVATIVENESS_ENABLE_PER_TECH_BEHIND_IN_TECH_PENALTY = 1, -- Enable that the penalty is applied for each tech we are behind in tech.
	INNOVATIVENESS_AHEAD_OF_TIME_BONUS = 0.02, -- Bonus for being ahead of time in tech
	INNOVATIVENESS_DAYS_AFTER_FIRST_PICK_VIABLE = 90, -- How many days you cna still get innov
	INNOVATIVENESS_MAX = 1000.0, -- Innovativeness Cap
	...
	SETTLMENT_GROWTH_CHANCE_MULTIPLIER = 0.25, -- Affects chance of development increase per year if a colonist is working on Settlement Growth.
	SETTLMENT_GROWTH_CHANCE_MIN = 0.01, -- Minimum chance of increasing development for colonists promoting Settlement Growth
	SETTLEMENT_GROWTH_CHECK_INTERVAL = 30, -- Interval in days between checks for random development increase when working in Settlement Growth.
...
NEconomy = {
	...
	EDICTS_DURATION_MONTHS = 60,				-- months lastin at least.
	...
	GOLD_MINE_DEPLETION_THRESHOLD = 5,				-- Gold mines above production level or above can be depleted
	GOLD_MINE_DEPLETION_CHANCE = 0.2,					-- Chance of gold mine being depleted (yearly, per production above threshold)
	...
	GOLD_INFLATION = 0.75,							-- _EDEF_GOLD_INFLATION_
	TREASURE_FLEET_INFLATION = 0.5,
	BASE_YEARLY_INFLATION = 0,					-- yearly inflation increase
	INFLATION_FROM_LOAN = 0.2,						-- increase per loan
...
NMilitary = {
	...
	FORT_DEVASTATION_IMPACT = -20,				-- multiplied by fortlevel/max fortlevel in area per year.
	...
	REGULAR_LEADER_POWER = 2,						-- Used for generals & admirals (affects total pips)
	...
	SIEGE_ATTRITION = 1,
	SIEGE_DISEASE_IMPACT = 0.1,						-- Fraction of besieging soldiers lost in a disease outbraek
	...
	BASE_COMBAT_WIDTH = 10.0,						-- _MDEF_BASE_COMBAT_WIDTH_
	MAX_COMBAT_WIDTH = 50,
	FORCE_MARCH_FACTOR = 0.5,						--
	LOOT_DEVASTATION_IMPACT = 40,						-- how much devastation full loot impacts
	...
	CANNOT_RETREAT_DAYS = 10, 						-- How many days combat has to last before one side can retreat
	DAYS_PER_PHASE = 5,								-- How many days each fire and shock phase lasts
	DAYS_PER_SIEGE_PHASE = 30,						-- How many days each siege phase lasts
	COMBAT_DICE_SIDE = 4,							-- How many sides the dice used in combat has
	...
	LEADER_MAX_PIPS = 10,								-- Max general/admiral pips (per skill)
	LEADER_MIN_PIPS = 4,								-- Min general/admiral pips (total)
	...
	SEA_LANDING_PENALTY = -3,							-- Dice roll penalty from sea invasion
	MOTHBALLING_MONTHLY_DECREASE = 0.05,				-- When mothballin is turned on, the units strength will lose this much each month
	MOTHBALLING_MINIMUM_STRENGTH = 0.1,				-- The minimum strength value the fleet will reach while mothballin
	MIN_RECRUIT_TIME_MODIFIER = 0.2,					-- Recruit time cannot be lower than this * base
	PRESTIGE_FROM_LAND = 15,					-- Maximum base prestige from land battles (unmodified).
	PRESTIGE_FROM_NAVAL = 15,					-- Maximum base prestige from naval battles (unmodified).
	...
	INF_LOOT = 0.15,								-- How many ducats/month will a single full strength infantry regiment loot?
	CAV_LOOT = 0.33,								-- How many ducats/month will a single full strength cavalry regiment loot?
	...
	MAX_REBEL_SUPPRESSION = 20.0;				-- Maximum amount of rebel suppression by troops in a province
	INF_SUPPRESSION = 0.1,						-- The amount of reduction to unrest for each friendly infantry regiment in a province
	CAV_SUPPRESSION = 0.1,						-- The amount of reduction to unrest for each friendly cavalry regiment in a province
	ART_SUPPRESSION = 0.1,						-- The amount of reduction to unrest for each friendly artillery regiment in a province
...
NAI = {
	ACCEPTABLE_BALANCE_DEFAULT = 1.3, --AI wants this advantage to enter battles typically. (There are some exceptions, e.g. offensives.)
	ACCEPTABLE_BALANCE_FRIEND_IN_COMBAT = 0.5,
	EDICT_VALUE_THRESHOLD = 20, -- The higher this value, the less the AI will use Edicts
	...
	AGGRESSIVENESS = 1000, -- Base chance (out of 10000) of AI being willing to start a war each diplomatic tick (~1.5 times a month)
	...
	FORCE_COMPOSITION_CHANGE_TECH_LEVEL = 12, -- Tech level at which AI will double its artillery fraction
